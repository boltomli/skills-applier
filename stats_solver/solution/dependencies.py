...
class DependencyGenerator:
    ...
    STANDARD_LIBRARY = {
        "os", "sys", "math", "random", "statistics", "itertools",
        "collections", "functools", "datetime", "json", "csv",
        "re", "string", "pathlib", "typing", "dataclasses",
    }
...
setup(
    ...
    def check_conflicts(self, dependencies: List[str]) -> List[str]:
        ...
        packages = [self.PACKAGE_ALIASES.get(d.lower(), d).split(">=")[0].split("==")[0].split("~=")[0] for d in dependencies]
        
        for pkg1, pkg2 in known_conflicts:
            if pkg1 in packages and pkg2 in packages:
                conflicts.append(f"Potential conflict: {pkg1} and {pkg2} may not work well together")
