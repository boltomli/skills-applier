...
class OutputFormatRecognizer:
    ...
    FORMAT_PATTERNS = {
        OutputFormat.PLOT: [
            r"plot", r"graph", r"visualize", r"visualization", r"figure",
            r"matplotlib", r"seaborn", r"plotly",
        ],
        OutputFormat.HISTOGRAM: [
            r"histogram", r"distribution", r"freq(uency)?\s+hist",
        ],
        OutputFormat.SCATTER_PLOT: [
            r"scatter", r"scatter\s+plot", r"point\s+plot",
        ],
        OutputFormat.LINE_PLOT: [
            r"line\s+plot", r"line\s+graph", r"trend\s+line",
        ],
        ...
        OutputFormat.TABLE: [
            r"table", r"tabular", r"data\s+table",
        ],
        OutputFormat.DATAFRAME: [
            r"dataframe", r"pandas\s+df", r"df\s*=|df\.",
        ],
        OutputFormat.CSV: [
            r"csv", r"\.csv", r"comma\s+separated",
        ],
        OutputFormat.NUMBER: [
            r"number", r"value", r"result", r"single\s+value",
        ],
        OutputFormat.PERCENTAGE: [
            r"percent", r"%", r"percentage", r"proportion",
        ],
        OutputFormat.TEXT: [
            r"text", r"string", r"message", r"output\s+text",
        ],
        OutputFormat.REPORT: [
            r"report", r"document", r"summary\s+report",
        ],
        OutputFormat.SUMMARY: [
            r"summary", r"summarize", r"brief",
        ],
        OutputFormat.EXPLANATION: [
            r"explain", r"explanation", r"describe", r"interpret",
        ],
        OutputFormat.BOOLEAN: [
            r"boolean", r"bool", r"binary",
        ],
        OutputFormat.YES_NO: [
            r"yes\s*/\s*no", r"yes\s+or\s+no", r"answer\s+yes\s+or\s+no",
        ],
        OutputFormat.TRUE_FALSE: [
            r"true\s*/\s*false", r"true\s+or\s+false",
        ],
        OutputFormat.P_VALUE: [
            r"p\s*[-_]?\s*value", r"p-value", r"p\s*=\s*",
        ],
        OutputFormat.CONFIDENCE_INTERVAL: [
            r"confidence\s+interval", r"ci\s*[0-9]?", r"95%\s+ci",
        ],
        OutputFormat.STATISTIC: [
            r"statistic", r"test\s+statistic", r"t-stat", r"f-stat",
        ],
        OutputFormat.IMAGE: [
            r"image", r"png", r"jpg", r"jpeg", r"save\s+figure",
        ],
        OutputFormat.PDF: [
            r"pdf", r"\.pdf", r"pdf\s+report",
        ],
        OutputFormat.FILE: [
            r"file", r"save\s+to\s+file", r"write\s+to\s+file",
        ],
        OutputFormat.PRINT: [
            r"print", r"output", r"display", r"show",
        ],
    }
...
- optional_elements: Array of elements that could optionally be included"""
    ...
    def get_format_requirements(self, output_format: OutputFormat) -> Dict[str, Any]:
        ...
        return requirements.get(output_format, {
            "libraries": [],
            "typical_elements": [],
            "file_extensions": [],
        })
