...
class AlternativeFinder:
    ...
    async def find_alternatives(
        ...
        # 1. Similar methods
        similar = await self._find_similar_methods(primary_skill, all_skills, problem_type)
        alternatives.extend(similar)

        # 2. Simpler alternatives
        simpler = await self._find_simpler_alternatives(primary_skill, all_skills, problem_type)
        alternatives.extend(simpler)

        # 3. More advanced methods
        advanced = await self._find_more_advanced(primary_skill, all_skills, problem_type)
        ...
        complementary = await self._find_complementary(primary_skill, all_skills, problem_type)
    ...
    async def _find_complementary(
        ...
        if primary_skill.category == SkillCategory.STATISTICAL_METHOD:
            ...
            for skill in all_skills:
                ...
                if any(
                    kw in skill.id.lower() or kw in skill.description.lower() for kw in viz_keywords
                ):
                    alt = Alternative(
                        skill=skill,
    ...
    def _calculate_similarity(self, skill1: SkillMetadata, skill2: SkillMetadata) -> float:
        ...
        if skill1.statistical_concept and skill2.statistical_concept:
            if skill1.statistical_concept == skill2.statistical_concept:
                ...
                skill1.statistical_concept in skill2.statistical_concept
                or skill2.statistical_concept in skill1.statistical_concept
            ):
                score += 0.1
    ...
    def _is_compatible_problem_type(self, skill: SkillMetadata, problem_type: ProblemType) -> bool:
