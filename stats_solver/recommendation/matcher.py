...
class SkillMatcher:
    ...
    async def match(
        ...
        for skill in skills:
            match_result = await self._match_single_skill(
                skill, problem_features, problem_type, data_type_result, output_format
            )
    ...
    async def _match_single_skill(
        ...
        # Problem type match
        problem_type_score, problem_type_reason = self._match_problem_type(skill, problem_type)
        ...
        total_score = sum(score * self.WEIGHTS.get(key, 0) for key, score in scores.items())
    ...
    def _match_output_format(
        ...
        if any(
            kw in skill.description.lower() or kw in " ".join(skill.tags).lower() for kw in keywords
        ):
