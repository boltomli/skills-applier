...
class PrerequisiteChecker:
    ...
    def _infer_implicit_prerequisites(
        ...
        if skill.category == SkillCategory.STATISTICAL_METHOD:
            if not any(
                "descriptive" in s.id.lower()
                for s in available_skills
                if s.id in skill.prerequisites
            ):
                descriptive_skills = [
                    s.id
                    for s in available_skills
        ...
        if "regression" in skill.id.lower():
            if not any(
                "correlation" in s.id.lower()
                for s in available_skills
                if s.id in skill.prerequisites
            ):
                correlation_skills = [
                    s.id for s in available_skills if "correlation" in s.id.lower()
                ]
        ...
        if "scipy" in skill.dependencies:
            scipy_skills = [
                s.id for s in available_skills if "scipy" in s.dependencies and s.id != skill.id
            ]
    ...
    def get_missing_prerequisites(
        ...
        for result in results:
            missing_for_skill = [
                p for p in result.prerequisites if p.status == PrerequisiteStatus.MISSING
            ]
    ...
    def generate_prerequisite_report(
        ...
        return {
            ...
            "skills_with_missing": [r.skill.id for r in results if r.missing_count > 0],
        }
