...
class SkillChainBuilder:
    ...
    def __init__(
        ...
        prereq_checker: Optional[PrerequisiteChecker] = None
    ) -> None:
        ...
        if step_type == ChainStepType.CORE_ANALYSIS:
            ...
            for skill in available_skills:
                if skill.id != core_skill.id:
                    if any(kw in skill.id.lower() or kw in skill.description.lower() for kw in preparation_keywords):
                        skills.append(skill)
            ...
            for skill in available_skills:
                if skill.id != core_skill.id:
                    if any(kw in skill.id.lower() or kw in skill.description.lower() for kw in validation_keywords):
                        skills.append(skill)
            ...
            for skill in available_skills:
                if skill.id != core_skill.id:
                    if any(kw in skill.id.lower() or kw in skill.description.lower() for kw in post_keywords):
                        skills.append(skill)
            ...
            if problem_features.requires_visualization or skill in core_skill.tags:
                ...
                for skill in available_skills:
                    if skill.id != core_skill.id:
                        if any(kw in skill.id.lower() or kw in skill.description.lower() or kw in " ".join(skill.tags).lower() for kw in viz_keywords):
                            skills.append(skill)
    ...
    def _determine_dependencies(
        ...
        if any(s.id == skill.id and s.step_type == ChainStepType.CORE_ANALYSIS for s in existing_steps):
    ...
    def _calculate_chain_confidence(
        self,
        steps: List[ChainStep],
        prereq_results: List[PrerequisiteCheckResult]
    ) -> float:
